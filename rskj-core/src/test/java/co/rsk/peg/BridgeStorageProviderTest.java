...
public class BridgeStorageProviderTest {
    ...
    public void createSaveAndRecreateInstance() throws IOException {
        ...
        Assert.assertEquals(Coin.MILLICOIN, feePerKb);
    }
    ...
    public void getActiveFederation() throws IOException {
        ...
        Federation activeFederation = new Federation(Arrays.asList(new BtcECKey[]{BtcECKey.fromPrivate(BigInteger.valueOf(100))}), Instant.ofEpochMilli(1000), 0L, NetworkParameters.fromID(NetworkParameters.ID_REGTEST));
    ...
    @Test
    public void getFeePerKbElection_nonNullBytes() {
        AddressBasedAuthorizer authorizerMock = mock(AddressBasedAuthorizer.class);
        Repository repositoryMock = mock(Repository.class);
        BridgeStorageProvider storageProvider = new BridgeStorageProvider(repositoryMock, "aabbccdd", RskSystemProperties.CONFIG.getBlockchainConfig().getCommonConstants().getBridgeConstants());

        Coin electionFee = Coin.FIFTY_COINS;
        HashSet<TxSender> electionVotes = new HashSet<>();
        byte[] serializedElection = BridgeSerializationUtils.serializeFeePerKbElection(
                new FeePerKbElection(authorizerMock, electionVotes, electionFee));
        ...
        FeePerKbElection result = storageProvider.getFeePerKbElection(authorizerMock);
        assertThat(result.getVotes(), is(electionVotes));
        assertThat(result.getFeePerKb(), is(electionFee));
        assertThat(result.getWinner().isPresent(), is(false));
    }
